          ...
          <IonTabs>
            <IonRouterOutlet>
              <Route path="/automobiles/:id" exact={true}>
                <Automobiles switcher = {state} aidi = {idAssigner}/>

              </Route>
              <Route path="/drivers/:id" exact={true} >
                <Drivers switcher = {state} aidi = {idAssigner}/>
                
              </Route>
              {/* <Route exact path="/" render={() => <Redirect to={`/automobiles/${userId}`}/>}/> */}
            </IonRouterOutlet>
            <IonTabBar slot="top">
              <IonTabButton tab="automobiles" href={`/automobiles/${userId}`}>
                <IonIcon  icon={carSportOutline} />
                <IonLabel>Автомобили</IonLabel> 
              </IonTabButton>
              <IonTabButton tab="drivers" href={`/drivers/${userId}`}>
